...
@cross_origin() 
def search_results(model, query, page):
    if model == 'events':
        events = Event.query
        ...
        return jsonify(query_filter_by_page([e for e in events]), page)
    elif model == 'jobs':
        jobs = Job.query
        ...
        return jsonify(query_filter_by_page([j for j in jobs], page))
    elif model == 'cities':
        cities = City.query
        cities = cities.filter(or_(City.name.like('%' + query + '%'),
                                   City.state.like('%' + query + '%')))
        return jsonify(query_filter_by_page([c for c in cities], page))
    elif model == 'all':
        events = Event.query
        events = events.filter(or_(Event.name.like('%' + query + '%'),
                                   Event.summary.like('%' + query + '%'),
...
@cross_origin()
def sort_results(model, attribute, page):
    if model == 'events':
        events = Event.query
...
@cross_origin()
def desc_sort_results(model, attribute, page):
    if model == 'events':
        events = Event.query
        if attribute == 'eventid':
